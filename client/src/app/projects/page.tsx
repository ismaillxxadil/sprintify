      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="card">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-primary">Current projects</h2>
              <p className="text-sm text-secondary">Open a project dashboard to manage backlog, sprints, and members.</p>
            </div>
            <span className="rounded-full bg-surface-secondary px-3 py-1 text-xs font-semibold text-secondary">
          ...
          {loading ? (
            ...
            <p className="text-sm text-secondary">You do not have active projects yet.</p>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-secondary">{project.description || 'No description provided.'}</p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-2 text-xs text-secondary md:grid-cols-3">
        ...
        <div className="space-y-6">
          <section className="card">
            ...
            <p className="mt-1 text-sm text-secondary">Create a project and become its PO automatically.</p>
          ...
          <section className="card">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-secondary">Accept an invite to add the project to your workspace.</p>
              </div>
              <span className="rounded-full bg-surface-secondary px-3 py-1 text-xs font-semibold text-secondary">
            ...
            {loading ? (
              <p className="text-sm text-secondary">Loading invites...</p>
            ) : invites.length === 0 ? (
              <p className="text-sm text-secondary">You have no pending invites.</p>
            ) : (
              <div className="space-y-4">
                {invites.map((invite) => (
                  <div key={invite.projectId} className="rounded-2xl border p-4" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h3 className="font-semibold text-primary">{invite.projectName}</h3>
                        <p className="mt-1 text-sm text-secondary">Role: {invite.role}</p>
                        <p className="mt-1 text-xs text-secondary">Invited: {formatDateTime(invite.invitedAt)}</p>
                      </div>
